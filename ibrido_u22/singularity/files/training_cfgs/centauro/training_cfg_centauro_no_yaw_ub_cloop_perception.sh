...
export COMMENT='centauro big wheels (fixed ankle yaw) with upper body CLOOP, ' # any training comment
...
ground_type enable_height_sensor height_shared_grid height_shared_resolution enable_height_vis"
export CUSTOM_ARGS_DTYPE="float bool bool float bool bool bool bool bool bool bool bool bool bool str bool int float bool"
export CUSTOM_ARGS_VALS="0.1 false false 0.1 true true true false true true false false false true stepup true 10 0.16 false"
export SET_ULIM=1 
export ULIM_N=28672 # maximum number of open file descriptors for each process (shared memory)
export TIMEOUT_MS=15000 # timeout after which each script autokills ([ms])
