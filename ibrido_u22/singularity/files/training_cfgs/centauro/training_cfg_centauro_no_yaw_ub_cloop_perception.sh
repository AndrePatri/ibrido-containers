...
export COMMENT='centauro big wheels (fixed ankle yaw) with upper body CLOOP, 16392 bsize, UTD 4, 1/3 replay bsize, HUGE n envs, entropy annhealing, 5-15cm steps, 1-2 steps,  smaller platforms (4m), full flight control (apex, end[+-], length), no yaw rate randomization, track/CoT (0.6 scale, 0.3 offset)/a rate, logstd log(5), 0.35 max step height' # any training comment
