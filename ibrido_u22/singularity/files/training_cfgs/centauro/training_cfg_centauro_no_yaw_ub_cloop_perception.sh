...
export COMMENT='centauro big wheels (fixed ankle yaw) with upper body CLOOP, 8192 bsize, 1/3 replay bsize, clipping rewards lb, separate discrete/cont targets (-0.2, -1.5), actions bounds refactor, flight info fix/addition, no rand pert, 10cm steps, smaller platforms (4m), steup with coll prims, heightmap obs, flight control, no omega tracking, UTD 8, track/CoT (x6 scale, x3 offset)/a rate, logstd log(5), 0.2 max step height' # any training comment
