...
export RNAME="CentauroCloopPartialNoYaw" # a descriptive base name for this run
...
export COMMENT='centauro big wheels (fixed ankle yaw) CLOOP partial, joint pof0 fix (5%),, UTD 8, action rep 3, fake pos track (action rate, CoT, dir track, max_vref 1.0 m/s, max omega 0.4 rad/s,), trgt entropy -0.5' # any training comment
...
export JNT_IMP_CONFIG_PATH="${HOME}/ibrido_ws/src/CentauroHybridMPC/centaurohybridmpc/config/jnt_imp_config_no_yaw_open.yaml" # path to yaml file for jnt imp configuration
if (( $IS_CLOSED_LOOP )); then
  export JNT_IMP_CONFIG_PATH="${HOME}/ibrido_ws/src/CentauroHybridMPC/centaurohybridmpc/config/jnt_imp_config_no_yaw.yaml"
fi
...
export CUSTOM_ARGS_NAMES="use_random_pertub use_jnt_v_feedback step_height control_wheels fixed_flights adaptive_is lin_a_feedback closed_partial fix_yaw use_flat_ground estimate_v_root self_collide"
export CUSTOM_ARGS_DTYPE="bool bool float bool bool bool bool bool bool bool bool bool "
export CUSTOM_ARGS_VALS="true true 0.1 true true true false true true true false false"
