...
if [ "$world_env_profile" = "isaac5x" ]; then
  ...
  # genesis world interface runs in its own numpy>=2 env (no pinocchio); the cluster/training below
  # stay on the base env. The two processes communicate over EigenIPC shared memory.
  (
