...
export SHM_NS="centauro_big_wheels_yaw_ub" # shared mem namespace used for all shared data on CPU 
export N_ENVS=1 # number of env to run in parallel
export RNAME="CentauroCloopYawUb" # a descriptive base name for this run
...
export COMMENT='centauro big wheels yaw upper body CLOOP' # any training comment
...
if [[ $RT_DEPLOY -eq 1 ]]; then
  ...
  export CUSTOM_ARGS_VALS="true 0.10 true true true false true false true 1.0 false true true false 1.0 fast true"
  ...
  export CUSTOM_ARGS_VALS="0.10 false 60.0 true true true false false $TIMEOUT_MS $HOME/ibrido_ws/src/$XMJ_FILES_DIR true true false true 1.0 fast true"
