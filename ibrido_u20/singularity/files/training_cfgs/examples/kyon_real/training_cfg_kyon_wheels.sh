...
if [[ $RT_DEPLOY -eq 1 ]]; then
  export XBOT_CONFIG="KyonRLStepping/kyonrlstepping/config/xmj_env_files/kyon_real/xbot2_basic_wheels_real.yaml"
else
  export XBOT_CONFIG="KyonRLStepping/kyonrlstepping/config/xmj_env_files/kyon_real/xbot2_basic_wheels.yaml"
fi
...
export SHM_NS="kyon_real_wheels" # shared mem namespace used for all shared data on CPU 
export N_ENVS=1 # number of env to run in parallel
export RNAME="KyonRealCloopPartialWheels" # a descriptive base name for this run
...
export COMMENT='kyon wheels real cloop partial, ' # any training comment
...
if [[ $RT_DEPLOY -eq 1 ]]; then
  export CUSTOM_ARGS_NAMES="wheels fix_yaw fixed_flights adaptive_is lin_a_feedback closed_partial fully_closed estimate_v_root base_linkname use_diff_vels xmj_timeout xmj_files_dir state_from_xbot rt_safety_perf_coeff use_mpc_pos_for_robot" 
  export CUSTOM_ARGS_DTYPE="xacro bool bool bool bool bool bool bool str bool int string bool float bool"
  export CUSTOM_ARGS_VALS="true false true true false true false false pelvis false $TIMEOUT_MS $HOME/ibrido_ws/src/$XMJ_FILES_DIR true 1.0 true" 
  export REMOTE_ENV_FNAME="aug_mpc_envs.envs.xmj_env"  

else
  export CUSTOM_ARGS_NAMES="render_to_file render_fps wheels fix_yaw fixed_flights adaptive_is lin_a_feedback closed_partial fully_closed estimate_v_root base_linkname use_diff_vels xmj_timeout xmj_files_dir state_from_xbot" 
  export CUSTOM_ARGS_DTYPE="bool float xacro bool bool bool bool bool bool bool str bool int string bool"
  export CUSTOM_ARGS_VALS="false 60.0 true false true true false true false false pelvis false $TIMEOUT_MS $HOME/ibrido_ws/src/$XMJ_FILES_DIR false" 
  export REMOTE_ENV_FNAME="aug_mpc_envs.envs.xmj_env"  
fi
